...
fDS = 1;   % 1: with FD, 2: with SD, 3: with AD
noiterations = 1;

%% set the parameters for the SPA model
a = 0.1;
b = 0.6;
k = 1;
...
for i = 1:noiterations
    ...
    elseif fDS ==3
        ...
        fTLC = (Yin(1,:).^2.*Yin(2,:)-k*NNout(1,:).*Yin(1,:));
        gTLC = [a*Yin(2,:)+Yin(1,:).^2.*Yin(2,:)-Yin(1,:)+Yin(3,:).*NNout(1,:);        
                -a*Yin(2,:)-Yin(1,:).^2.*Yin(2,:)+b ];                                   
        dNNdyout = [Wout*(((Win(:,1)*ones(1,size(Yin,2)))).*(phiL.*(1-phiL)));
                    Wout*(((Win(:,2)*ones(1,size(Yin,2)))).*(phiL.*(1-phiL)))];

        fun = fTLC-Yin(3,:).*(dNNdyout(1,:).*gTLC(1,:)+dNNdyout(2,:).*gTLC(2,:));
        %% form problem
...
function [Fmin, Jac] = funPIloss(curLearn,netDim,X,fixedPar)
    ...
    b = fixedPar(2);
    k = fixedPar(3);
    dydtToyLC = ToyLCode_SP2(0.,[hfunNN; X(1:2,:)],X(3,:),a,b,k); % autonomous system
    f(1,:) = dydtToyLC(1,:).*X(3,:);       % f = dxdt * eps
    ...
    if nargout>1
        ...
        [Jac_x, ~] = gradToyLCode_SP(0.,[hfunNN; X(1:2,:)],X(3,:),a,b,k);
        gradfx = Jac_x(1,:).*X(3,:);
        gradgx = Jac_x(2:3,:);
